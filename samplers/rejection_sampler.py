...
def get_samples(y, eta, distribution : utils.densities.Distribution, num_samples, threshold=1e-3):
    n, d = y.shape[0], y.shape[-1]
    yk = y.repeat_interleave(num_samples,dim=0)
    samples, accepted_idx = get_rgo_sampling(
        yk, eta, distribution.log_prob, threshold, 
